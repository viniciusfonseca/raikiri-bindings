...
mod imports {
  ...
  pub struct CalledModuleResponse {
    pub status:u16,pub body:String,pub headers:Vec<ResponseHeader>,
  }
    ...
    unsafe {
      ...
      for i in 0..len6 {
        ...
        result6.push({
          ...
          ResponseHeader {
            name:String::from_utf8(Vec::from_raw_parts(*((base+0)as *const i32)as *mut _,len4,len4)).unwrap(),value:String::from_utf8(Vec::from_raw_parts(*((base+8)as *const i32)as *mut _,len5,len5)).unwrap(),
          }
        });
      }std::alloc::dealloc(base6 as *mut _,std::alloc::Layout::from_size_align_unchecked((len6 as usize)*16,4,),);
      CalledModuleResponse {
        status:i32::from(*((ptr2+0)as *const u16))as u16,body:String::from_utf8(Vec::from_raw_parts(*((ptr2+4)as *const i32)as *mut _,len3,len3)).unwrap(),headers:result6,
      }
    }
  }
  #[repr(align(4))]
...
mod exports {
  ...
  unsafe extern "C" fn __wai_bindgen_exports_handler(arg0:i32,arg1:i32,) -> i32 {
    ...
    let result =  <super::Exports as Exports>::handler(String::from_utf8(Vec::from_raw_parts(arg0 as *mut _,len0,len0)).unwrap());
    let ptr1 = EXPORTS_RET_AREA.0.as_mut_ptr()as i32;
    let ModuleResponse {
      status:status2,body:body2,headers:headers2,
    } = result;
    *((ptr1+0)as *mut u16) = (wai_bindgen_rust::rt::as_i32(status2))as u16;
